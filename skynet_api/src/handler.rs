...
use actix_cloud::{async_trait, memorydb::MemoryDB, Result};
use sea_orm::DatabaseTransaction;
use skynet_macro::default_handler;

use crate::{
    entity::{groups, notifications, permissions, users},
    permission::{PermEntry, PermissionItem, UserPerm},
    request::Condition,
    HyUuid, Skynet,
};
...
pub trait SettingHandler: Send + Sync {
    ...
    /// Get `name` setting, decode with base64.
    fn get_base64(&self, name: &str) -> Option<Vec<u8>>;

    /// Set setting `name` with `value`, encode with base64.
